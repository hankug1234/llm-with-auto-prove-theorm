...
if __name__ == "__main__":
    ...
    prem1 = Terminology("P", [Constant("a")])
    prem2 = (Operation.ALL, "x",
            (Operation.IMPLIE,
            Terminology("P", [Var("x")]),
            Terminology("Q", [Var("x")])))

    goal  = Terminology("Q", [Constant("a")])
    ...
    # ─────────────────────────────────────────────
    #  전제 S
    prem1 = Terminology("P", [Constant("a")])                          # ① P(a)
    prem2 = (
    ...
    goal  = Terminology("Q", [Constant("a")])    # entailment가 안 됨
    tableaus.append(prove_with_premises([prem1], goal, qdepth=5))
    # ----------------------------------------------
    # 3. ∧ 도입  P(a) ∧ R(a) ⊢ R(a)
    prem1 = (Operation.AND,
            Terminology("P", [Constant("a")]),
            Terminology("R", [Constant("a")]))
    goal  = Terminology("R", [Constant("a")])
    tableaus.append(prove_with_premises([prem1], goal, qdepth=5))
    # ----------------------------------------------
    ...
    prem1 = (Operation.NEG, (Operation.NEG, Terminology("P", [Constant("b")])))
    goal  = Terminology("P", [Constant("b")])
    tableaus.append(prove_with_premises([prem1], goal, qdepth=5))
    # ----------------------------------------------
    ...
    prem1 = ("neg",
            (Operation.AND,
            Terminology("P", [Constant("a")]),
            Terminology("Q", [Constant("a")])))
    goal  = (Operation.OR,
            ("neg", Terminology("P", [Constant("a")])),
            ("neg", Terminology("Q", [Constant("a")])))
    tableaus.append(prove_with_premises([prem1], goal, qdepth=5))
    # ----------------------------------------------
    ...
    prem2 = Terminology("P", [Constant("c")])
    goal  = (Operation.SOME, "y", Terminology("R", [Constant("c"), Var("y")]))
    tableaus.append(prove_with_premises([prem1,prem2], goal, qdepth=5))
    # ----------------------------------------------
